...
핸드오프는 한 에이전트가 대화의 일부를 다른 에이전트에 위임할 수 있게 합니다. 서로 다른 에이전트가 특정 영역에 특화되어 있을 때 유용합니다. 예를 들어 고객 지원 앱에서는 예약, 환불, FAQ를 처리하는 에이전트를 각각 둘 수 있습니다.
...
`handoff()` 함수는 생성되는 도구를 세부 조정할 수 있게 해 줍니다.
...
- `inputFilter` – 다음 에이전트로 전달할 히스토리를 필터링
...
기본적으로 핸드오프는 전체 대화 히스토리를 받습니다. 다음 에이전트에 전달되는 내용을 조정하려면 `inputFilter`를 제공하세요.
