...
음성 에이전트는 OpenAI 음성-음성(speech-to-speech) 모델을 사용해 실시간 음성 채팅을 제공합니다. 이들 모델은 스트리밍 오디오, 텍스트, 도구 호출을 지원하며, 음성/전화 고객 지원, 모바일 앱 경험, 음성 채팅과 같은 애플리케이션에 적합합니다.
...
<LinkCard
  ...
  description="OpenAI Agents SDK를 사용해 실시간 음성 어시스턴트를 단 몇 분 만에 만들 수 있습니다."
/>
...
- 스트리밍 이벤트에 대한 콜백
- 텍스트 및 음성 에이전트에 동일한 구성 요소 재사용

음성-음성 모델을 사용하면, 모델이 동작한 후 텍스트를 다시 오디오로 변환하기 위해 음성을 전사하고 재변환할 필요 없이 모델의 실시간 오디오 처리 능력을 활용할 수 있습니다.
