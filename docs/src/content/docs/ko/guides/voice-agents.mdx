...
Voice Agents는 OpenAI 음성-음성 모델을 사용하여 실시간 음성 채팅을 제공합니다. 이러한 모델은 스트리밍 오디오, 텍스트, 도구 호출을 지원하며 음성/전화 고객 지원, 모바일 앱 경험, 음성 채팅과 같은 애플리케이션에 적합합니다.
...
<LinkCard
  ...
  description="OpenAI Agents SDK를 사용하여 단 몇 분 만에 첫 실시간 음성 어시스턴트를 구축하세요."
/>
...
- 스트리밍 이벤트용 콜백
- 텍스트 및 음성 에이전트 모두에서 동일한 구성요소 재사용
